...
contract TrustableFund {
    ...
    modifier checkUniqueFunderaise() {
        ...
        uint latestID;
        if (length != 0) latestID = fundRecords[msg.sender][length - 1];

        require(
            length == 0 || fundList[latestID].active == false,
            "You already have one openning Fundraise"
        );
        _;
    }
    ...
    modifier canBeWithdrawed() {
        ...
        require(
            (fundList[latestID].active == true &&
                fundList[latestID].deadLine < block.timestamp &&
                fundList[latestID].donation > 0) ||
                (fundList[latestID].donation >= fundList[latestID].goal),
    ...
    function donation(uint _fundID, string memory _time) public payable {
    ...
    function getDonationHistoryByUser()
        public
        view
        returns (DonationHistory[] memory, bool)
    {
    ...
    function getFundraiseByUser()
        public
        view
        returns (Fundraise[] memory, bool)
    {
    ...
    function getDonationByFundraise(
        uint _fundID
    ) public view returns (DonationHistory[] memory, bool) {
        ...
        DonationHistory[] memory history = new DonationHistory[](
            donationIndexes.length
        );
    ...
    function withdrawFundraise() public payable canBeWithdrawed {
        uint length = fundRecords[msg.sender].length;
        uint latestID = fundRecords[msg.sender][length - 1];
    ...
    function cancelFundraise(
        string memory _time
    ) public payable canBeCancelled {
        uint length = fundRecords[msg.sender].length;
        uint latestID = fundRecords[msg.sender][length - 1];
    ...
    function getFundTitle(uint _fundID) public view returns (string memory) {
        ...
        return fundList[_fundID].storyTitle;
    }
}
